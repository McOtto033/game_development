...
document.addEventListener('click',event=>{
  ...
  if(action==='export-card'){download(`card-spec-${d.id}.json`,{...M.exportCard(d),definitions:M.clone(defs().filter(def=>Object.values(d.card.skills).some(sk=>sk.effects.some(e=>e.typeId===def.id))))});return;}
  ...
  if(action==='add-trait')d.card.traits.push({name:'',value:'',description:''});
  ...
  if(action==='cell'){const t=s.effects[i].target,side=button.dataset.side,cell=Number(button.dataset.cell);t[side]=t[side].includes(cell)?t[side].filter(x=>x!==cell):[...t[side],cell].sort((a,b)=>a-b);markDirty();button.classList.toggle('selected');button.setAttribute('aria-pressed',String(t[side].includes(cell)));renderPreview();return;}
  if(action==='target-preset') {
    const t=s.effects[i].target,key=button.dataset.preset;t.ally=[];t.enemy=[];
