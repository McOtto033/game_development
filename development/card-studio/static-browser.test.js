...
const {editEffects,assertEffects}=require('./effect-browser-checks');
...
(async()=>{
  ...
  try {
    ...
    await fs.writeFile(path.join(artifactDir,'static-results.json'),JSON.stringify({passed:true,checks:['サブパス配信','端末内保存と再読込','タブ競合の保護','前回バックアップ出力','独立端末へのJSON取込','対象マスタップと390px表示','強化弱体する能力の独立指定','行動効果による対象指定','3回攻撃を1効果として保存','必殺技3効果の追加複製並替と上限','1440/820/390/320px表示','公開先への書込通信なし'],webkitTested:false},null,2));
